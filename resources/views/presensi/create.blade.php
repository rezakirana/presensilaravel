            ...
            <form role="form" method="post" action="{{ route('presensi.store') }}">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="exampleInputJK">Jadwal</label>
                        <select class="form-control" name="jadwal_id" id="jadwal_id" required>
                           @foreach ($dataJadwal as $key => $jadwal)
                           <option value ="{{ $jadwal->id }}">{{ $jadwal->kelas->nama_kelas.' - '.$jadwal->mapel->nama_mapel.' - '.$jadwal->jam_pelajaran }}</option>
                           @endforeach
                        </select>
                    </div>
